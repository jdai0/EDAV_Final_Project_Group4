...
<script>
  ...
  function showImage(borough) {
    ...
    renderBarChart(borough, 'OFNS_DESC');
  }
  ...
  function renderBarChart(boroughName, variable) {
    ...
    const aggregatedData = crimes.reduce((acc, crime) => {
      const crimeVariable = crime[variable]; // Use the passed variable
...
First, when hovering over a specific bar, the full name of arrest types (which could be too long to be fully incorporated in a normal bar chart) and the exact arrest numbers would be displayed, and the bar would be highlighted. This adds information without making the graph messing and hard to read. 

Second, putting geographic distribution alongside the interactive bar plots is helpful. It provides contextual insights through emphasizing where the borough in discussion is located on the scope of NYC, and how crimes within the borough is distributed. 

Lastly, this combination provides a basis for a detailed portfolio of crime analysis divided by boroughs, which could be a future working direction. Currently, we have only incorporated geographic information and Arrest Types due to the complexity of transforming data into readable form for javascript. However, with the flexibility of javascript functions, we could easily use them to create bar plots for other categorical variables. We could add new functions for other graph types and add textual descriptions, further enriching the portfolios. This would provide a thorough view for the differences of boroughs in terms of criminal activities.
</div>
