...
Let's first see the rough age & race distribution
...
```{r, echo=FALSE, results='hide', warning=FALSE}
heatmap_data <- data_demo %>%
  count(AGE_GROUP, PERP_RACE) %>% 
  arrange(AGE_GROUP, PERP_RACE)
head(heatmap_data)
```
...
#| fig-width: 10
#| fig-height: 8
ggplot(heatmap_data, aes(x = AGE_GROUP, y = PERP_RACE, fill = n)) +
  ...
  labs(
    title = "Age and Race Interaction Heatmap",
    x = "Age Group",
    y = "Race"
  ) +
  theme_minimal() +
